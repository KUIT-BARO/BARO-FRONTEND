...
const PostPromiseShare = async (codeList: string[], promiseId: number) => {
  try {
    return await instance.post(`/promise/share/${promiseId}`, { codeList });
  } catch (error) {
    if (error.response) {
...
export default PostPromiseShare;
